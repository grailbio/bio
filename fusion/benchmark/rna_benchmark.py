...
def run_starfusion(sample_name: str, cached_file_pairs: List[util.FASTQPair], args: Any):
...
def run_af4(sample_name: str, cached_file_pairs: List[util.FASTQPair], args: Any):
    ...
    for mode in ['denovo', 'targeted']:
        ...
        if mode == 'targeted':
            af4_args.append(f'-cosmic-fusion={args.cache_dir}/all_pair_art_lod_gpair_merged.txt')
...
def main() -> None:
    ...
    for sample in util.RNA_SAMPLES:
        ...
        for fp in sample.paths:
            ...
            cached_file_pairs.append(util.FASTQPair(r1=args.cache_dir + '/' + os.path.basename(fp.r1),
