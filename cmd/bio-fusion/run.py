...
def main() -> None:
    ...
    p.add_argument('--transcript', default='/scratch-nvme/fusion/transcriptome.fa',
    ...
    if 'go' in config.run:
        ...
        if config.update_gene_list:
            argv = [f'-transcript={config.transcript}', f'-gene-list-output={gene_list}']
            if config.mode != RunMode.DENOVO:
